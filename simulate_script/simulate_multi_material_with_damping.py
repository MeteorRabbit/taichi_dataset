...
def run_simulation(output_dir="workspace/taichi/output_sim", material_type='non_newtonian'):
    ...
    dt = 1e-5 # Reduced from 1e-4 to 1e-5 to handle the high stiffness (E=5e8) without CFL violation
    frame_dt = 1/60.0
    ...
    if is_cpu:
        ...
        dx = 0.01 # High resolution for GPU (Improved from 0.02 to 0.01 to reduce floating gap)
    ...
    def get_material_props(m_type):
        ...
        if m_type == 'elasticity':
            m_id = MPMSimulator.elasticity
            rho = 20000.0 # Exaggerated density (was 10000) for heavy iron effect
            E = 1e9  # Increased to 1e9 (1 GPa) for very rigid iron
            nu = 0.25
            mu = E / (2 * (1 + nu))
            ...
            E = 5e5   # Lowered to 5e5 (from 1e6) to make sand easier to displace/push aside
            nu = 0.3
            mu = E / (2 * (1 + nu))
            lam = E * nu / ((1 + nu) * (1 - 2 * nu))
    ...
    target_ground_particles = 100000 
    ...
    for frame in range(simulation_frames): 
        ...
        # [DEBUG] Print Duck Minimum Y
        min_y_duck = pos_obj[:, 1].min()
        print(f"Frame {frame}: Duck Min Y = {min_y_duck:.6f} (Sand Surface ~ 0.0)")
