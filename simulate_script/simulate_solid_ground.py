# 网格版本：鸭子模型使用原本mesh顶点，落到刚体碰撞体地面上，最后恢复成原有的网格模型
...
class MPMSimulator:
    ...
    def __init__(self, dtype, dt, frame_dt, particle_layout, dx, inv_dx, n_particles, gravity=[0, -9.8, 0], material=elasticity, cuda_chunk_size=400):
        ...
        # Buffer for efficient data export (avoids OOM with to_numpy() on large dynamic fields)
        self.x_out = ti.Vector.field(dim, dtype=self.dtype, shape=2**20) # Max 1M particles

    @ti.kernel
    def copy_x_to_buffer(self, n: ti.i32, s: ti.i32):
        for i in range(n):
            self.x_out[i] = self.x[i, s]

    def fetch_x(self, n, s=0):
        self.copy_x_to_buffer(n, s)
        return self.x_out.to_numpy()[:n]
    ...
    def check_cfl(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            if ti.math.isnan(self.v[p, s]).any():
                print("Particle Velocity NaN at ", p)
                self.cfl_satisfy[None] = 0
            if ti.math.isnan(self.x[p, s]).any():
                print("Particle Position NaN at ", p)
                self.cfl_satisfy[None] = 0
            
            # Bound check (Grid limit: [-20.48, 20.48])
            # Use specific limit slightly smaller than grid boundary
            limit = 20.45 
            pos = self.x[p, s]
            if pos[0] < -limit or pos[0] > limit or \
               pos[1] < -limit or pos[1] > limit or \
               pos[2] < -limit or pos[2] > limit:
                print("Particle out of bounds at ", p, " pos: ", pos)
                self.cfl_satisfy[None] = 0
...
def load_mesh_vertices_and_faces(filename, scale=1.0, offset=[0.5, 0.5, 0.5]):
    ...
    if isinstance(mesh, trimesh.Scene):
        ...
        # Combine all geometries or take the first one
        # Here we take the first one for simplicity, or concatenate
...
def run_simulation(output_dir="workspace/taichi/output_sim", material_type='elasticity'):
    ...
    initial_pos_np = sim.fetch_x(num_particles[None], 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output_dir', type=str, default="workspace/taichi_dataset/particles_output/output_solid_ground", help="Output directory for simulation results")
