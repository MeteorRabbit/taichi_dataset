...
OUTPUT_DIR = os.path.join(BASE_DIR, "render_output/soft_hard")
...
def main(should_render=False):
    print(f"Soft-Hard Rendering Started. Mode: {'RENDER' if should_render else 'PREVIEW'}")
    ...
    for frame in range(0, max_frame + 1):
        ...
        for i, cam in enumerate(cameras):
            ...
            all_data.append({
                "file_path": f"./soft_hard/{filename}",
                "time": time,
                "c2w": get_c2w(cam),
