...
def load_mesh_particles(filename, dx, scale=1.0, offset=[0.5, 0.5, 0.5]):
    ...
    # Center the mesh at (0,0,0) first then move to offset
    # mesh.apply_translation(-mesh.centroid)
    # mesh.apply_translation(offset)
    
    # Or just apply offset directly if the mesh is already centered or user handles it
    # Let's assume user wants to place the mesh at 'offset'
    # But voxelization is grid aligned.
    
    # Voxelize
    # pitch is the voxel size. Using dx/2 for denser packing than simulation grid if desired, 
    # but dx is fine for 1-to-1 mapping.
    print("Voxelizing mesh...")
    voxel_grid = mesh.voxelized(pitch=dx) 
    
    # Fill interior to make it solid
    print("Filling interior...")
    voxel_grid = voxel_grid.fill()
    
    points = voxel_grid.points.astype(np.float32)
    
    # Apply offset after voxelization to place it in the simulation domain
    # Note: voxel_grid.points are in the mesh's original coordinate system (after scaling)
    points += np.array(offset, dtype=np.float32)
    
    print(f"Generated {len(points)} particles from mesh.")
    return points
...
def run_simulation():
    ...
    # Option 1: Water (Viscous Fluid)
    material = MPMSimulator.viscous_fluid
    rho = 1000.0
    mu = 1.0
    kappa = 1000.0 # Bulk modulus
    lam = kappa - 2/3 * mu # For fluid
    
    # Option 2: Elastic (Jelly/Rubber) - Uncomment to use
    ...
    # Option 3: Sand (Drucker-Prager) - Uncomment to use
    ...
    # Option 4: Non-Newtonian Fluid (Cream/Toothpaste) - Uncomment to use
    ...
    # lam = kappa - 2/3 * mu
    # yield_stress = 10.0
    # plastic_viscosity = 1.0 # or 0.1 for toothpaste
    
    # Initialization
    ply_path = "workspace/taichi/toyduck.ply" # Set this to your PLY file path, e.g., "/path/to/model.ply"
    ...
    if ply_path and os.path.exists(ply_path):
        ...
        init_pos = load_mesh_particles(ply_path, dx=dx*0.5, scale=1.0, offset=[0.5, 0.5, 0.5])
    ...
    # Run loop
    output_dir = "workspace/taichi/output_sim"
    os.makedirs(output_dir, exist_ok=True)
    
    print("Starting simulation...")
    for frame in range(120): # Run for 120 frames
        sim.advance(frame)
